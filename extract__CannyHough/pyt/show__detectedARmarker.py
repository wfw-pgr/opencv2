...
def show__detectedARmarker( inpFile=None, nARmarker=4, frame_to_use=None, outFile="out.jpg" ):
    ...
    # ------------------------------------------------- #
    resize       = None
    images       = lmf.load__movieFrames( inpFile=inpFile, designate=True, \
...
if ( __name__=="__main__" ):
    ...
    outFile   = "jpg/out_{0:04}.jpg"
    frame_to_use = 3

    images = []
    for iframe in range( 100 ):
        show__detectedARmarker( inpFile=inpFile, nARmarker=nARmarker, \
                                frame_to_use=iframe, outFile=outFile.format( iframe ) )
